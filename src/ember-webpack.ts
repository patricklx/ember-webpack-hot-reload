...
const Webpack: PackagerConstructor<Options> = class Webpack implements Packager {
    ...
    currentBuildPromiseResolve: (value: void | PromiseLike<void>) => void;
    lastWebpackServer: any;
        ...
        return {
            ...
            plugins: [
                ...
                new HtmlWebpackPlugin({
                    chunks: Object.keys(entry).find(e => !e.endsWith('/test.js')),
    ...
    private getWebpack(appInfo: AppInfo) {
        ...
        let config = this.variants.map((variant, variantIndex) =>
            mergeWith({}, this.configureWebpack(appInfo, variant, variantIndex), devServerOptions, this.extraConfig, appendArrays)
        );
        this.lastAppInfo = appInfo;
        this.lastWebpack = webpack(config);
        if (config[0].devServer.enabled) {
            let options = { ...devServerOptions };
            delete options.devServer.enabled;
            const server = new WebpackDevServer(options, this.lastWebpack);
            server.start();
